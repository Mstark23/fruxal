  ...
  try {
    ...
    for (const biz of businesses) {
      ...
      for (const step of SEQUENCE) {
        ...
        const { data: existing } = await sb.from("outreach_log").select("id").eq("businessId", (biz as any).business_id).eq("template", step.template).limit(1);
        if (existing && existing.length > 0) { skipped++; continue; }

        // Get user email
        const { data: member } = await sb.from("business_members").select("userId, users(email)").eq("businessId", (biz as any).business_id).limit(1).single();
        if (!member?.users) { skipped++; continue; }
        const email = (member.users as any).email;

        // Get leak data for template
        const { data: rawLeaks } = await (sb as any)
          .from("detected_leaks")
          .select("title, annual_impact_min, severity, description")
          .eq("business_id", (biz as any).business_id)
          .order("annual_impact_min", { ascending: false });
        const leakRows: any[] = rawLeaks || [];
        const totalLeaking = leakRows.reduce((s: number, l: any) => s + (l.annual_impact_min ?? 0), 0);
        const dailyLoss = Math.round(totalLeaking / 365);

        // Build email content
        const { emailTemplate, sendEmail } = await import("@/services/email/service");
        let body = "";
        let subject = step.subject.replace("{dailyLoss}", `$${dailyLoss}`).replace("{industry}", (biz as any).industry_label || (biz as any).industry || "business");

        switch (step.template) {
          case "quick_wins":
            const top3 = leakRows.slice(0, 3);
            body = `<p>Here are your 3 quickest wins to start saving money today:</p>
              ${top3.map((l, i) => `<p><strong>${i + 1}. ${l.title}</strong> — $${l.annual_impact_min?.toLocaleString()}/yr<br/><span style="color:#666">${l.description || "Fix this to save money"}</span></p>`).join("")}
              <p>Total potential savings: <strong style="color:#00c853">$${top3.reduce((s, l) => s + (l.annual_impact_min ?? 0), 0).toLocaleString()}/yr</strong></p>`;
            break;
          case "urgency":
            body = `<p>Every day you wait, you're losing approximately <strong style="color:#ff3d57">$${dailyLoss}</strong>.</p>
              ...
              <p>Your biggest leak — <strong>${leakRows[0]?.title || "unknown"}</strong> — alone costs $${(leakRows[0]?.annual_impact_min ?? 0).toLocaleString()}/yr.</p>`;
            break;
          case "case_study":
            body = `<p>A ${(biz as any).industry_label || (biz as any).industry || "business"} just like yours used Fruxal to find $47,200 in annual leaks. Within 30 days, they had fixed their top 3 issues and were already saving $18,400/yr.</p>
              <p><strong>Their #1 fix?</strong> Renegotiating their payment processing rate — a 20-minute phone call that saved $7,200/yr.</p>
              <p>You have ${leakRows.length} open leaks totaling $${(totalLeaking ?? 0).toLocaleString()}/yr. What if you fixed just one this week?</p>`;
            break;
          case "discount":
            body = `<p>We noticed you haven't upgraded yet, so here's something special:</p>
              ...
              <p>With Pro you get: all ${leakRows.length} leaks unlocked, fix recommendations, AI intelligence, reports & exports.</p>
        ...
        const html = emailTemplate(subject, body, "Open Dashboard", `${process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_APP_URL || "https://fruxal.vercel.app"}/v2/dashboard`);
        await sendEmail({ to: email, subject, html });

        // Log it
        await sb.from("outreach_log").insert({
          id: `outreach_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
          businessId: (biz as any).business_id,
