...
export default function ObligationsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/v2/obligations/calendar?${params}`);
          ...
          <div className="space-y-5">
            {[
              ...
              return <TimelineSection key={sec.title} title={sec.title} items={sec.items} accentColor={sec.accent}
                lang={lang} onComplete={(s,ti,p)=>setModal({slug:s,title:ti,penalty_max:p})}
                onSnooze={snooze} defaultOpen={sec.open} isCont={sec.cont} />;
            })}
          </div>
        )}
