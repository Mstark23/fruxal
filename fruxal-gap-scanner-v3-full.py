    ...
    for i, line in enumerate(lines):
        if ".toLocaleString()" in line:
            if "?." not in line and "|| 0" not in line and "?? 0" not in line:
                if not re.search(r'\d+\.toLocaleString', line) and 'if (' not in line:
...
def chk_interval_leak(p, content, lines):
    if "setInterval" in content:
        ...
        elif "return () =>" not in content and "useEffect" in content:
...
def chk_timeout_leak(p, content, lines):
    if "setTimeout(" in content and "clearTimeout" not in content and "useEffect" in content:
        add("LOW","Memory/TimeoutLeak", p, 0,
            "setTimeout in useEffect without clearTimeout — may update state after unmount")
...
def chk_n_plus_one(p, content, lines):
    ...
    for i, line in enumerate(lines):
        if re.search(r'(for\s*\(|\.forEach\(|\.map\()', line):
            ...
            if re.search(r'await.*\.(from|findFirst|findMany|findUnique)\(', ctx):
...
def chk_missing_key(p, content, lines):
    if not is_page(p) and not is_comp(p): return
    for i, line in enumerate(lines):
        if ".map((" in line or ".map(function" in line:
            ctx = "".join(lines[i:min(i+6, len(lines))])
            if "key=" not in ctx and "key =" not in ctx and "<>" not in ctx:
...
def chk_window_ssr(p, content, lines):
    ...
    for i, line in enumerate(lines):
        if re.search(r'\bwindow\b', line) and "typeof window" not in line:
            ctx = "".join(lines[max(0,i-8):i])
            if "useEffect" not in ctx and "use client" not in content[:200]:
...
def keep(r):
    # Webhooks/register legitimately have no user auth
    if r["cat"] == "Security/NoAuth":
        legit_noauth = ["webhook", "register", "prescan/bridge", "prescan/save",
                       ...
                       "notifications/route", "partners/route"]
        if any(x in r["file"] for x in legit_noauth):
            return False
    # Admin routes use their own auth middleware (rep-auth or API key)
    if r["cat"] == "Security/NoAuth" and "admin/tier3" in r["file"]:
        return False
