...
//   - Context is now fetched server-side — page just calls GET /api/v2/chat?lang=X
//   - Loading state: "Reviewing your financials..." (1-2s before chat opens)
//   - Welcome message generated by Claude (personalised, not hardcoded)
//   - Quick reply pills (context-aware, disappear once user types)
//   - Context refreshed when fruxal:task:completed fires (Build 03 integration)
//   - Static STARTERS replaced by dynamic quick replies
//
// UNCHANGED from original:
//   - Auth flow, language toggle, paywall gate
//   - Message rendering, TypingDots, conversation history
//   - Input bar, send on Enter, new-chat button
//   - Enterprise context bar with score/findings
...
function ReviewingState() {
  return (
    <div className="flex items-center gap-2 py-4">
      ...
      <p className="text-[12px] text-ink-muted">Reviewing your financials…</p>
    </div>
  );
}
...
export default function ChatPage() {
  ...
  const [ctxLoading, setCtxLoading]   = useState(true); // "Reviewing your financials..."
  const [error, setError]             = useState<string|null>(null);
  const [paywalled, setPaywalled]     = useState(false);
  const [welcomeMsg, setWelcomeMsg]   = useState<string>("");
  const [quickReplies, setQuickReplies] = useState<string[]>([]);
  const [ctxDirty, setCtxDirty]       = useState(false); // needs refresh before next send
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Fetch lightweight client context for the enterprise context bar only
        ...
        if (dashRes?.data) {
          diagCtx.businessName = dashRes.data.profile?.business_name;
          diagCtx.industry     = dashRes.data.profile?.industry;
          diagCtx.province     = dashRes.data.profile?.province;
        }
        if (reportRes?.data) {
  ...
  // ── Fetch welcome + quick replies (server-generated context) ───────────────
  const fetchWelcome = useCallback(async (currentLang: "en"|"fr") => {
    setCtxLoading(true);
    try {
      const res = await fetch(`/api/v2/chat?lang=${currentLang}`);
      if (!res.ok) return;
      const data = await res.json();
      if (data.welcome) setWelcomeMsg(data.welcome);
      if (data.quickReplies?.length) setQuickReplies(data.quickReplies);
    } catch {
      // non-fatal — chat still works without welcome
    } finally {
      setCtxLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (!loading) fetchWelcome(lang);
  }, [loading, lang, fetchWelcome]);

  // ── Listen for task completion — mark context dirty ─────────────────────────
  useEffect(() => {
    const handler = () => setCtxDirty(true);
    window.addEventListener("fruxal:task:completed", handler);
    return () => window.removeEventListener("fruxal:task:completed", handler);
  }, []);
  ...
  useEffect(() => {
    if (!loading && !ctxLoading) inputRef.current?.focus();
  }, [loading, ctxLoading]);
  ...
  const send = useCallback(async (text?: string) => {
    ...
    // If context is dirty (task just completed), refresh welcome silently
    if (ctxDirty) {
      setCtxDirty(false);
      fetchWelcome(lang).catch(() => {});
    }

    setInput("");
    setQuickReplies([]); // hide quick replies once user starts chatting
    ...
    try {
      const res = await fetch("/api/v2/chat", {
        ...
        body: JSON.stringify({
          ...
          language: lang,
          // NOTE: clientContext no longer used for system prompt (built server-side)
  ...
  }, [input, sending, userId, convId, lang, ctxDirty, fetchWelcome, t]);
  ...
  if (error) return (
    <div className="min-h-screen bg-bg flex items-center justify-center px-6">
      <div className="text-center max-w-xs">
        ...
        <button onClick={() => window.location.reload()} className="px-4 py-2 text-[11px] font-semibold text-white rounded-lg" style={{ background:"#1B3A2D" }}>
    ...
    <div className="bg-bg min-h-screen font-sans flex flex-col" style={{ height:"100dvh" }}>
      ...
      <div className="flex items-center justify-between px-6 lg:px-8 py-4 bg-white border-b border-border-light shrink-0"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[13px] font-semibold text-ink leading-none">{t("Fruxal AI","Fruxal IA")}</p>
          ...
          <div className="flex bg-bg-section border border-border-light rounded-[7px] p-[3px] gap-[2px]">
            {(["en","fr"] as const).map(l => (
              <button key={l} onClick={() => { setLang(l); try { localStorage.setItem("fruxal_lang", l); } catch { /* non-fatal */ } }}
                className={`px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wide rounded-[5px] transition-all ${lang === l ? "bg-white text-ink shadow-sm" : "text-ink-muted"}`}>
                {l.toUpperCase()}
              </button>
            ))}
          </div>
          {messages.length > 0 && (
            <button onClick={() => { setMessages([]); setConvId(null); setWelcomeMsg(""); setQuickReplies([]); fetchWelcome(lang); }}
          ...
          {showEmpty && (
            <div className="pt-4 pb-8">
              <div className="mb-5">
                ...
                {/* Loading state — "Reviewing your financials..." */}
                {ctxLoading ? (
                  <ReviewingState />
                ) : welcomeMsg ? (
                  /* Claude-generated welcome message */
                  <div className="bg-white border border-border-light rounded-2xl rounded-bl-sm px-4 py-3 text-[12px] text-ink leading-relaxed max-w-[78%]"
                    style={{ boxShadow:"0 1px 3px rgba(0,0,0,0.03)" }}>
                    {welcomeMsg}
                  </div>
                ) : (
              ...
              {/* Quick reply pills — disappear when input is used */}
              {!ctxLoading && quickReplies.length > 0 && !input && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {quickReplies.map((q, i) => (
                    <button key={i} onClick={() => send(q)}
                      className="px-3 py-2 text-[11px] font-medium bg-white border border-border-light rounded-full hover:border-brand/30 hover:bg-brand/[0.02] transition-all text-ink-secondary hover:text-ink"
                      style={{ boxShadow:"0 1px 3px rgba(0,0,0,0.03)" }}>
                      {q}
                    </button>
                  ))}
          ...
          <div className="space-y-4">
            ...
            {sending && (
              <div className="flex justify-start">
                ...
                <div className="bg-white border border-border-light rounded-2xl rounded-bl-sm" style={{ boxShadow:"0 1px 3px rgba(0,0,0,0.03)" }}>
                  <TypingDots />
                </div>
              </div>
            )}
          ...
          {paywalled ? (
            ...
            <div className="flex items-end gap-3 bg-bg border border-border-light rounded-xl px-4 py-3"
              ...
              <textarea
                ref={inputRef}
                value={input}
                onChange={e => { setInput(e.target.value); if (e.target.value) setQuickReplies([]); }}
                onKeyDown={onKey}
                rows={1}
                placeholder={isEnt
