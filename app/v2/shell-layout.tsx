...
export default function ShellLayout({ children }: { children: React.ReactNode }) {
  ...
  // ── FIX: always start false (SSR-safe), read localStorage in useEffect only ──
  const [isEnterprise, setIsEnterprise] = useState(false);

  useEffect(() => {
    const onEnterprisePath = ENTERPRISE_PATHS.some(
  ...
  const NAV      = isEnterprise ? NAV_ENTERPRISE : NAV_STANDARD;
  const navRoot  = isEnterprise ? "/v2/dashboard/enterprise" : "/v2/dashboard";
        ...
        <div className="px-3 pb-4">
          <div className="border-t border-border-light pt-3 space-y-0.5">
            <button onClick={() => router.push("/v2/settings")}
              className={`w-full text-left flex items-center gap-2.5 px-3 py-[9px] rounded-sm transition-all ${
                normPath?.startsWith("/v2/settings")
                  ? "bg-brand-soft text-brand font-semibold"
                  : "text-ink-muted hover:bg-bg-warm hover:text-ink-secondary"
              }`}>
          ...
          <div className="mt-3 px-3 py-2.5 bg-bg rounded-sm border border-border-light flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              {isLoading ? (
                <>
                  <div className="h-2.5 w-20 bg-border-light rounded animate-pulse mb-1" />
                  <div className="h-2 w-28 bg-bg-section rounded animate-pulse" />
                </>
              ) : (
                <>
                  <p className="text-ink text-[11px] font-semibold truncate">{user?.name || user?.email}</p>
                  <p className="text-ink-faint text-[9px] truncate">{user?.email}</p>
                </>
              )}
            </div>
          </div>
