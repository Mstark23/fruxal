...
export default function LandingPage() {
  ...
  // Set US country cookie on mount
  useEffect(() => {
    document.cookie = "fruxal_country=US; path=/; max-age=31536000; samesite=lax";
  }, []);
  ...
  const startPrescan = async () => {
    ...
    try {
      const res = await fetch("/api/v3/prescan-chat", {
        ...
        body: JSON.stringify({ sessionId: null, message: "Hi, I'm ready to start.", history: [], lang: "en", country: "US", prefilledIndustry: prefilledIndustry || undefined }),
      });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/v3/prescan-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId, message: text, history, lang: "en", country: "US" }),
      });
      const data: PrescanChatResponse = await res.json();
    ...
    <div className="min-h-screen bg-bg font-sans text-ink">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-[60px] px-6 lg:px-12 flex items-center justify-between bg-bg/85 backdrop-blur-xl border-b border-border-light">
        ...
        <div className="hidden md:flex items-center gap-9">
          ...
          <a href="/pricing" className="text-sm font-medium text-ink-secondary hover:text-ink transition">Pricing</a>
          <a href="/faq" className="text-sm font-medium text-ink-secondary hover:text-ink transition">FAQ</a>
          <a href="/partners" className="text-sm font-medium text-ink-secondary hover:text-ink transition">Partners</a>
      ...
      <section className="pt-[100px] pb-20 px-6 lg:px-12 max-w-[1280px] mx-auto grid lg:grid-cols-[1fr_440px] gap-16 items-center min-h-screen">
        ...
        <div className="pt-5">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-brand-soft rounded-full text-xs font-semibold text-brand tracking-wide mb-7">
            ...
            {t("Trusted by 500+ US businesses · $12M+ in leaks identified", "Trusted by 500+ US businesses · $12M+ in leaks identified")}
          </div>
          <h1 className="font-serif text-h1 text-ink font-normal mb-5 max-w-[560px]">
            {t("Most US businesses are quietly losing ", "Most US businesses are quietly losing ")}
            <em className="italic text-brand-accent">{t("$50K–$150K a year.", "$50K–$150K a year.")}</em>
            {t(" Most owners have no idea where.", " Most owners have no idea where.")}</h1>
          ...
          <div className="flex gap-10">
            {[
              { v: "$18,000", l: t("Avg. leak / year", "Avg. leak / year") },
      ...
      <section id="features" className="py-24 px-6 lg:px-12 max-w-[1200px] mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border-light border border-border-light rounded-card overflow-hidden">
          {[
            { n: "01", h: t("Leak detection", "Détection de fuites"), p: t("Compares your costs against benchmarks for your industry, state, and revenue level.", "Compares your costs against benchmarks for your industry, state, and revenue level.") },
      ...
      <section id="pricing" className="py-24 px-6 lg:px-12 max-w-[1200px] mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 items-start">
          ...
          <div className="relative bg-white border border-border-light rounded-[16px] p-8 flex flex-col">
            ...
            <p className="text-sm text-ink-secondary mb-6 leading-relaxed">
              {t(
                "For US businesses doing $1M+. A dedicated Fruxal analyst runs a full forensic audit — covering federal and state tax credits, payroll, nexus, R&D credits — paid only on recovery.",
            ...
            <ul className="mb-8 flex-1">
              {([
                ...
                [t("IRS & compliance exposure", "Exposition ARC et conformité"), t("Audit risk, payroll remittances, provincial obligations.", "Risque de vérification, remises salariales, obligations provinciales.")],
                [t("R&D Credit & grant maximization", "Maximisation R&D Credit et subventions"), t("R&D credits, federal and provincial grants you qualify for.", "Crédits R&D, subventions fédérales et provinciales admissibles.")],
