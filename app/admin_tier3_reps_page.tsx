...
export default function RepsPage() {
  ...
  async function sendPortalAccess(repId: string) {
    setSendingPortal(true);
    setPortalSent(false);
    try {
      const r = await fetch("/api/rep/auth/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ repId }),
      });
      const d = await r.json();
      if (d.success) setPortalSent(true);
          ...
          <div className="relative w-full max-w-[480px] h-full bg-[#FAFAF8] border-l border-[#EEECE8] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {loadingDetail && !selectedRep ? (
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => { setPortalSent(false); sendPortalAccess(selectedRep.rep.id); }}
                    disabled={sendingPortal}
                    className="flex-1 py-2 rounded-xl text-xs font-bold disabled:opacity-40 transition"
                    style={{ background: "linear-gradient(135deg, #1B3A2D, #2D7A50)", color: "#fff" }}
                  >
                    {sendingPortal ? "Sending…" : "Send Portal Access →"}
                  </button>
                  {portalSent && (
                    <span className="text-[10px] font-semibold text-[#2D7A50]">✓ Sent</span>
                  )}
                </div>
