...
export async function GET(req: NextRequest) {
  try {
    ...
    // Determine language from query param or default to EN
    const langParam = req.nextUrl.searchParams.get("lang") || "en";
    const isFR = langParam === "fr";
    ...
    text(isFR ? "Rapport de fuites d'affaires" : "Business Leak Report", M, y, 14, font, gray); y -= 16;
    ...
    const stats = [
      { n: `${leaks.length}`, l: isFR ? "Fuites trouvees" : "Leaks Found", c: red },
      { n: `$${(potentialLoss ?? 0).toLocaleString()}`, l: isFR ? "Perte potentielle/an" : "Potential Loss/yr", c: red },
      { n: `$${(savedAmount ?? 0).toLocaleString()}`, l: isFR ? "Deja economise" : "Already Saved", c: green },
      { n: `${score}`, l: isFR ? "Score de sante" : "Health Score", c: teal },
    ];
    for (let i = 0; i < 4; i++) {
      const bx = M + i * (bw + 10);
    ...
    // Leak cards
    text(isFR ? `Fuites detectees (${leaks.length})` : `Leaks Detected (${leaks.length})`, M, y, 13, fontBold, dark); y -= 18;

    if (leaks.length === 0) {
      text(isFR ? "Aucune fuite detectee." : "No leaks detected.", M, y, 10, font, gray); y -= 20;
    }
    ...
    for (const leak of leaks) {
      ...
      if (isFix) {
        const ft = isFR ? "Corrige" : "Fixed"; const fw = fontBold.widthOfTextAtSize(ft, 8);
      ...
      const rt = isFR
        ? `Impact estime: $${(leak.impact_min ?? 0).toLocaleString()} - $${(leak.impact_max ?? 0).toLocaleString()}/an`
        : `Estimated impact: $${(leak.impact_min ?? 0).toLocaleString()} - $${(leak.impact_max ?? 0).toLocaleString()}/yr`;
    ...
    page.drawRectangle({ x: M, y: y - 1, width: CW, height: 1, color: light }); y -= 18;
    const f1 = isFR ? "Genere par Fruxal - fruxal.vercel.app" : "Generated by Fruxal - fruxal.vercel.app";
    text(f1, M + (CW - font.widthOfTextAtSize(f1, 8)) / 2, y, 8, font, gray); y -= 12;
    const f2 = isFR ? "Les resultats sont des estimations basees sur les references de l'industrie." : "Results are estimates based on industry benchmarks.";
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${isFR ? "rapport-fuites" : "leak-report"}-${d}.pdf"`,
